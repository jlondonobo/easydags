...
def test_running_ok():
    ...
    nodes.append(ExecNode(id_="f0", exec_function=example0))

    def example1():
        print("beginning 1")
        print("end 1")

    nodes.append(ExecNode(id_="f1", exec_function=example1, depends_on_soft=["f0"]))

    dag = DAG(
        nodes, name="NO HTML OUTPUT TEST", max_concurrency=8, debug=False, draw=False
    )
